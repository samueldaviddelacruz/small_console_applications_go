...
func (r *dbRepo) Create(i pomodoro.Interval) (int64, error) {
	...
	res, err := insStmt.Exec(i.StartTime.Format(time.DateOnly), i.PlannedDuration,
...
func (r *dbRepo) Update(i pomodoro.Interval) error {
	...
	res, err := updStmt.Exec(i.StartTime.Format(time.DateOnly), i.ActualDuration, i.State, i.ID)
	if err != nil {
		return err
	}
...
func (r *dbRepo) CategorySummary(day time.Time,
	...
	err := r.db.QueryRow(stmt, filter, day.Format(time.DateOnly)).Scan(&ds)
