...
func newWidgets(ctx context.Context, errorCh chan<- error) (*widgets, error) {
	...
	w.updateTtxtInfo = make(chan string)
	...
	w.txtInfo, err = newText(ctx, w.updateTtxtInfo, errorCh)
	if err != nil {
		return nil, err
	}
